...
class SearchExample {
  ...
  SearchExample(ShowDialogFunction showDialogCallback,
    ...
    _camera.lookAtPointWithMeasure(
        GeoCoordinates(10.7845766, 106.6212572), mapMeasureZoom);
  ...
  void _geocodeAnAddress() {
    ...
    GeoCoordinates geoCoordinates = GeoCoordinates(10.7845766, 106.6212572);
  ...
  Future<MapMarker> _addPoiMapMarker(GeoCoordinates geoCoordinates) async {
    ...
    if (_poiMapImage == null) {
      Uint8List imagePixelData = await _loadFileAsUint8List('MapMarker.png');
      _poiMapImage =
...
//////////////////////////////////////////////////////////
  ...
  void drawGPS(GeoCoordinates hehe) async {
    _clearMap();
    if (_poiMapImage == null) {
      Uint8List imagePixelData =
          await _loadFileAsUint8List('images/MapMarker.png');
      _poiMapImage =
          MapImage.withPixelDataAndImageFormat(imagePixelData, ImageFormat.png);
    }
    MapMarker mapMarker = MapMarker(hehe, _poiMapImage!);
    _hereMapController.mapScene.addMapMarker(mapMarker);
    _mapMarkerList.add(mapMarker);

    MapMeasure mapMeasureZoom = MapMeasure(MapMeasureKind.distance, 8000);
    _hereMapController.camera.lookAtPointWithMeasure(hehe, mapMeasureZoom);
  }
